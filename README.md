...
## PyBoss Challenge
This challenge uses the `employee_data.csv` which currently holds employee records like the one below:
...
```
The script converts and export the data to use the following format instead:
...
```
* In summary, the script performs the following conversions:

  * The `Name` column is split into separate `First Name` and `Last Name` columns.

  * The `DOB` column is re-written into `MM/DD/YYYY` format.
...
* Special Hint: To re-write the abbreviations next link eas helpful—[Python Dictionary for State Abbreviations](https://gist.github.com/afhaque/29f0f4f37463c447770517a6c17d08f5).


## PyParagraph Challenge
In this challenge, you get to play the role of chief linguist at a local learning academy. As chief linguist, you are responsible for assessing the complexity of various passages of writing. Having read so many passages, you've since come up with a fairly simple set of metrics for assessing complexity.

The task is to create a Python script to automate the analysis of any such passage using these metrics. The script does the following:
...
* **Special Hint:** Regular expressions are very helpful into completing this task. For example:
